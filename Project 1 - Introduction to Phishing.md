...
The **Introduction to Phishing** lab on **TryHackMe’s SOC Simulator** is designed to familiarize analysts with the core workflow of detecting, analyzing, and responding to phishing-based alerts in a simulated enterprise SOC environment.  

In this scenario, I acted as a **Tier 1 SOC Analyst**, responsible for investigating an **inbound phishing email alert** detected by the organization’s SIEM. The objective was to understand how phishing indicators appear in security logs, how to validate the alert, and how to document findings for escalation or closure.  
...
The first step in the investigation began with reviewing the SOC Simulator dashboard, where a **new alert** appeared under the *“Introduction to Phishing”* scenario. The screenshot below shows the initial alert summary generated by the SIEM.
...
The alert was titled **“Inbound Email Containing Suspicious External Link”** with a **Medium severity** rating and categorized under **Phishing**.

At this stage, I opened the alert to examine its key attributes, including the **subject line**, **sender domain**, **recipient address**, **message content**, and **embedded URL**. The alert description indicated that it was triggered by an **inbound email containing one or more external links** flagged as potentially suspicious by the SIEM’s detection rule.
...
| **Attachment** | None |
| **Direction** | Inbound |
| **Alert Status** | Awaiting Action → Assigned to self |

---

### 🕵️ Observations

- The **subject line** contains urgent language (“Action Required”), which is a **common phishing indicator** used to create a sense of urgency and prompt users to act quickly.  
- The **sender domain** (`hrconnex.thm`) mimics a legitimate HR-related domain but differs slightly from the recipient’s organization domain (`thetrydaily.thm`), which raises suspicion.  
- The **body of the email** includes a link labeled “Set Up My Profile,” which leads to a domain-hosted path `https://hrconnex.thm/onboarding/15400654060/j.garcia`.  
- No attachments were present, indicating this is a **URL-based phishing attempt** rather than an attachment-based one.  
- The SIEM rule suggests verifying whether any endpoints attempted to access the URL — the next step in confirming whether the threat was acted upon.
...
Using Splunk’s search query interface, I filtered the logs by the **sender email address**:
...
### 🧾 Verification Through Company Documentation
...
From the database:
- **Hannah Harris** — Human Resources (`h.harris@thetrydaily.thm`)  
...
Inside the Analyst VM, I found a built-in analysis utility called **TryDetectThis** — a simulated version of a URL and file reputation checker.  
...
**Result:**  
> ✅ **URL/IP Analysis Complete — Status: CLEAN**
...
- The alert was initially triggered due to **phishing-like characteristics**:  
  - Urgent tone in subject line (“Action Required”).  
...
### 🕒 Time of Activity
- **Date/Time:** November 25, 2025 – 03:37:23 UTC  
- **Event Summary:**  
...
1. **Alert Review:**  
   The initial alert in Splunk indicated a suspicious inbound email with an external link, categorized under “Phishing.”

2. **Log Analysis:**  
   - Reviewed all emails from the sender `onboarding@hrconnex.thm`.  
   - Found only two instances of outbound messages — both sent to the same recipient (`j.garcia@thetrydaily.thm`) within the last two hours.  
...
4. **URL Analysis:**  
   ...
   - Result: ✅ *URL/IP Analysis Complete — Status: CLEAN*  
...
This exercise demonstrates a complete Tier 1 SOC workflow — from alert triage to analysis, verification, and reporting.
